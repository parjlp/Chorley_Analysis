...
st.set_page_config(page_title="Forwards", layout="wide")
render_nav("forwards")

st.title("⚡ Forwards")

_, _, df_forwards, _ = load_data()
...
all_metrics = get_metric_cols(df_forwards)
...
with col1:
    min_minutes = st.slider(
        ...
        min_value=0,
        max_value=int(df_forwards['Minutes played'].max()),
        value=0,
        step=50
    )
with col2:
...
with st.expander("Select Metrics", expanded=False):
    ...
    with btn_col1:
        if st.button("Select All", key="fwd_select_all"):
            for m in all_metrics:
                st.session_state[f"fwd_metric_{m}"] = True
    with btn_col2:
        if st.button("Deselect All", key="fwd_deselect_all"):
            for m in all_metrics:
                st.session_state[f"fwd_metric_{m}"] = False

    cols = st.columns(4)
    selected_metrics = []
    for i, metric in enumerate(all_metrics):
        with cols[i % 4]:
            if st.checkbox(metric, value=st.session_state.get(f"fwd_metric_{metric}", False), key=f"fwd_metric_{metric}"):
                selected_metrics.append(metric)

filtered = df_forwards[df_forwards['Minutes played'] >= min_minutes]
